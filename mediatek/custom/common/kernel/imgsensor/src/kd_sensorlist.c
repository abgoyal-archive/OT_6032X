...
int Camera_debug_on=1;

static ssize_t CAM_debug_value_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    char *s = buf;
	    printk("[liwenyuan]CAM_debug_value_show s=%s\n",s);
    return snprintf(buf, PAGE_SIZE, "%d\n", Camera_debug_on);
}

static ssize_t CAM_debug_value_store(struct kobject *kobj, struct kobj_attribute *attr,
               const char *buf, size_t n)
{
    char *s = buf;
    char cmd_str[]="not set";
    printk("[liwenyuan]CAM_debug_value_store s=%s\n",s);
	    sscanf(buf, "%s", (char *)&cmd_str);
    printk("[liwenyuan]CAM_debug_value_store cmd_str=%s\n",cmd_str);
    if(strcmp(cmd_str, "off")==0)
    {
        printk("[liwenyuan]CAM_debug_value_store line=%s\n",__LINE__);
          Camera_debug_on=0;
   }
   else
   {
...
static DEVICE_ATTR(CAM_DEBUG_LEE, 0644,  CAM_debug_value_show, CAM_debug_value_store);  //0666->0664 


static struct attribute *CAM_sysfs_attrs[] = {
    &dev_attr_CAM_DEBUG.attr,
    &dev_attr_CAM_DEBUG_LEE.attr,
    NULL,
};
