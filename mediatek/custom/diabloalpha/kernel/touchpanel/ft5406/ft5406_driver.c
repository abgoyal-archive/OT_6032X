...
//End,[xiaoguang.xiong][2013.06.20][PR 475675]
...
//Begin,[xiaoguang.xiong][2013.07.26][PR 495884]
//BIEL module compatible design.
#define TS_TYPE_BIEL	0x3b

#define TS_MAX_UPDATE_COUNT 5
//End,[xiaoguang.xiong][2013.07.26][PR 495884]
...
//Begin,[xiaoguang.xiong][2013.07.22][489557]
...
#ifdef FW_UPGRADE_ENABLE
//begin,[xiaoguang.xiong][2013.07.22][PR 489557]
//add fw upgrade function when bootup,now the newest is V0B(2013.07.22)
...
//Begin,[xiaoguang.xiong][2013.07.31][498274]
//upgrade fw from 0b to 0d,to cover three problem:report point abnormal,current leakage when sleep mode,the frequency of report point is low.
#if 0
static unsigned char CTPM_TURLY_FW[]=
{
#include "DiabloAlpha_ID5a_V0B_20130426_app.h"
};
#endif
static unsigned char CTPM_TURLY_FW[]=
{
#include "DiabloAlpha_5406_ID5a_V0d_20130729_app.h"
};
//End,[xiaoguang.xiong][2013.07.31][498274]

//begin,[xiaoguang.xiong][2013.07.30][PR 497688]
//update fw of biel module,solve the problem:the menu key cannot work normal.
static unsigned char CTPM_BIEL_FW[]=
{
//#include "DiabloAlpha_ID3B_V12_20130506_app.h"
//#include "DiabloAlpha_5406_ID3B_V13_20130729_app.h"
#include "DiabloAlpha_5406_ID3B_V16_20131218_app.h"//modify by xiaoguang.xiong,2013.12.31,upgrade fw to v16(biel),CR 582451
};
//End,[xiaoguang.xiong][2013.07.30][PR 497688]
...
static u8 fts_ctpm_get_upg_ver(void)
{
	...
	if (tp_factory_id_cmd == TS_TYPE_TRULY) {
		...
		ui_sz = sizeof(CTPM_BIEL_FW);
		printk("fts_ctpm_get_upg_ver ui_sz,value1 value2 is %4x %2x %2x xxxxxxxxxx\n", \
		ui_sz,CTPM_BIEL_FW[ui_sz - 2],CTPM_BIEL_FW[ui_sz - 1]);
		if (ui_sz > 2) {    
			return CTPM_BIEL_FW[ui_sz - 2];
		} else {    
			/*TBD, error handling?*/
			return 0xff; /*default value*/
		}
...
//End,[xiaoguang.xiong][2013.07.22][PR 489557]
...
static int fts_ctpm_fw_upgrade_with_i_file(void)
{
	...
	if (tp_factory_id_cmd == TS_TYPE_TRULY) {
		...
		pbt_buf = CTPM_BIEL_FW;
		/*call the upgrade function*/
		i_ret =  fts_ctpm_fw_upgrade(pbt_buf,sizeof(CTPM_BIEL_FW));
		if (i_ret != 0)	{
			printk("[FTS] upgrade failed i_ret = %d.\n", i_ret);
		}
 		else {
...
//Begin,[xiaoguang.xiong][2013.07.22][PR 489557]
//add fw upgrade function when bootup, modify the timing when power on,follow D-FT5x06-1212-V4.0 ( FT5x06 Data Sheet ).pdf,page 13
	...
	mt_set_gpio_out(GPIO140, GPIO_OUT_ONE);D-FT5x06-1212-V4.0 ( FT5x06 Data Sheet ).pdf
	mdelay(100);
	mt_set_gpio_out(GPIO140, GPIO_OUT_ZERO);
	mdelay(10);
	hwPowerOn(MT65XX_POWER_LDO_VRF28_2, VOL_3000, "TP");//modify by ydyu, change TP VDD fron pre-proto
	mdelay(90);
...
if (update_retry == 0) {
	...
	printk("tp_factory_id_cmd = %x, read ID !\n", tp_factory_id_cmd); 
//modify by xiaoguang.xiong,2013.08.24,PR 513931.
//improve fw upgrade function when bootup, to cover the case:cannot read factory id when bootup.
#if 1	
	if ((tp_factory_id_cmd != TS_TYPE_TRULY) && (tp_factory_id_cmd != TS_TYPE_BIEL))
	{
		tp_factory_id_cmd = fts_ctpm_get_panel_factory_setting();
		printk("tp_factory_id = %x, read ID !\n", tp_factory_id_cmd);

                update_return = fts_ctpm_fw_upgrade_with_i_file();
			update_retry ++;				
			if ( (update_return != 0) && (update_retry < TS_MAX_UPDATE_COUNT)) {
				printk("restore TP firmware failed!\n");
				//goto reset_proc;
                                goto retry_update;
			}
	...
	if (tp_factory_id_cmd == TS_TYPE_TRULY) {
		...
		//if ((chip_fw_ver >= 0x90) && (chip_fw_ver < firmware_ver)) {
                if (chip_fw_ver < firmware_ver) {
			update_return = fts_ctpm_fw_upgrade_with_i_file();
			update_retry ++;				
			if ( (update_return!=0) && (update_retry < TS_MAX_UPDATE_COUNT)) {
