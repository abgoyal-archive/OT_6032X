			...
			u8 tx, u8 rx)
{
	...
	/*scan*/
    //modify by xiaoguang.xiong,2013.08.24,CR513917
    //modify raw data show function to fit MMI test.
        //err = read_reg(FTS_DEVICE_MODE_REG, &regvalue);
        err = ft5x0x_read_reg(i2c_client,FTS_DEVICE_MODE_REG,&regvalue);
	if (err < 0) {
		return err;
	} else {
		regvalue |= 0x80;
                //err = write_reg(FTS_DEVICE_MODE_REG, regvalue);
                err = ft5x0x_write_reg(i2c_client,FTS_DEVICE_MODE_REG,regvalue);
		if (err < 0) {
			return err;
		} else {
			for(i=0; i<20; i++)
	...
	for(i=0; i<tx; i++)
	{
		...
		writebuf[1] = i;
                ...
                ft5x0x_write_reg(i2c_client,writebuf[0],writebuf[1]);
		...
		for (j = 0; j < rx*2; j += 2)
        	{
                        ...
                        ft5x0x_read_reg(i2c_client,FTS_RAW_BEGIN_REG+j,(read_buffer+j));
                        ft5x0x_read_reg(i2c_client,FTS_RAW_BEGIN_REG+j+1,(read_buffer+(j+1)));
					...
					char *buf)
{
	...
	/*entry factory*/
        ...
        err = ft5x0x_write_reg(i2c_client,FTS_DEVICE_MODE_REG,FTS_FACTORYMODE_VALUE);
	if (err < 0) {
	...
	/*get rx and tx num*/
        ...
        err = ft5x0x_read_reg(i2c_client,FTS_TXNUM_REG,&tx);
	if (err < 0) {
        ...
        err = ft5x0x_read_reg(i2c_client,FTS_RXNUM_REG,&rx);
	if (err < 0) {
...
RAW_ERROR:
	/*enter work mode*/
        //write_reg(FTS_DEVICE_MODE_REG, FTS_WORKMODE_VALUE); //return to normal mode
        ft5x0x_write_reg(i2c_client,FTS_DEVICE_MODE_REG,FTS_WORKMODE_VALUE);
...
//begin,add by xiaoguang.xiong,2013.11.21,for diff data show function.CR558776
					...
					char *buf)
{
        char *s = buf;
	u16 before_rawdata[FTS_TX_MAX][FTS_RX_MAX] = {0};
	u16 after_rawdata[FTS_TX_MAX][FTS_RX_MAX] = {0};
	u8 orig_vol = 0x00;
	u8 rx = 0, tx = 0;
	u8 i = 0, j = 0;
	int err = 0;
	...
	err = ft5x0x_write_reg(i2c_client, FTS_DEVICE_MODE_REG, FTS_FACTORYMODE_VALUE);
	if (err < 0) {
                s += sprintf(s,"%s:,enter factory mode error, diffdata show error!\n", __func__);
		goto RAW_ERROR;
	}
	...
	err = ft5x0x_read_reg(i2c_client, FTS_TXNUM_REG, &tx);
	if (err < 0) {
                s += sprintf(s, "%s:diff data show,get tx error!\n",__func__);
		goto RAW_ERROR;
	}
	err = ft5x0x_read_reg(i2c_client, FTS_RXNUM_REG, &rx);
	if (err < 0) {
                s += sprintf(s, "%s:diff data show,get rx error!\n",__func__);
		goto RAW_ERROR;
	}
	...
	if (err < 0) {
                s +=sprintf(s, "%s:diff data show error 1!\n", __func__);
		goto RAW_ERROR;
	} 

	/*get original voltage and change it to get new frame rawdata*/
	err = ft5x0x_read_reg(i2c_client, FTS_VOLTAGE_REG, &regvalue);
	if (err < 0) {
                s +=sprintf(s, "%s:get original voltage error!\n", __func__);
		goto RAW_ERROR;
	} else 
		orig_vol = regvalue;
	...
	if (orig_vol <= 1)
        ...
        //s += sprintf(s,"original voltage: %u changed voltage:%u",orig_vol, regvalue);
	//s += sprintf(s,"\n");
	err = ft5x0x_write_reg(i2c_client, FTS_VOLTAGE_REG, regvalue);
	if (err < 0) {
                s +=sprintf(s, "%s:set original voltage error!\n", __func__);
		goto RAW_ERROR;
	}
	...
	if (err < 0) {
                s +=sprintf(s, "%s:diff data show error 2!\n", __func__);
...
RETURN_ORIG_VOLTAGE:
	err = ft5x0x_write_reg(i2c_client, FTS_VOLTAGE_REG, orig_vol);
	if (err < 0)
	{
		dev_err(&client->dev,
			"%s:return original voltage error!\n", __func__);
		s +=sprintf(s, "%s:return original voltage error!\n", __func__);
	}
          
RAW_ERROR:
	/*enter work mode*/
	err = ft5x0x_write_reg(i2c_client, FTS_DEVICE_MODE_REG, FTS_WORKMODE_VALUE);
	if (err < 0)
		dev_err(&client->dev,
			"%s:enter work error!\n", __func__);
	msleep(100);
...
//end,add by xiaoguang.xiong,2013.11.21,for diff data show function.CR558776
...
//add by xiaoguang.xiong,2013.11.21,for diff data show function.CR558776
