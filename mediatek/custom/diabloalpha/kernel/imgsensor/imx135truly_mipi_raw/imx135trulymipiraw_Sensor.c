...
static kal_bool debugFlag = 0;//KAL_FALSE; //zhfan for control log pr458701

static DEFINE_SPINLOCK(imx111_drv_lock);

//zhfan for control log pr458701
#define SENSORDB(fmt, arg...) if(debugFlag){printk( "[IMX135TRULYMIPIRaw] "  fmt, ##arg);}
#define RETAILMSG(x,...)
#define TEXT
...
//zhfan for control log pr458701
#define JRDSENSORDB(fmt, arg...) if(debugFlag){printk( "[JRDIMX135TRULYMIPIRaw] "  fmt, ##arg);}
#define JRDSENSORDB_ERROR(fmt, arg...) {printk( "[JRDIMX135TRULYMIPIRaw_ERROR] "  fmt, ##arg);}
...
#define IMX135TRULY_ID 0x02



#define IMX135TRULYOTP_LSC_FLAG_PAGE  0x0b
#define IMX135TRULYOTP_LSC_FLAG_SIZE 1
#define IMX135TRULYOTP_LSC_FLAG_ADDRESS 0x3b43

#define IMX135TRULYOTP_LSC_START_PAGE  0x04
#define IMX135TRULYOTP_LSC_SIZE 504
#define IMX135TRULYOTP_LSC_START_ADDRESS 0x3b04

#define IMX135TRULYOTP_MDI_START_PAGE 0x01,0x02,0x03
#define IMX135TRULYOTP_MDI_SIZE 1
#define IMX135TRULYOTP_MDI_START_ADDRESS 0x3b05

#define IMX135TRULYOTP_AWB_START_PAGE 0x01,0x02,0x03
#define IMX135TRULYOTP_AWB_SIZE 23
#define IMX135TRULYOTP_AWB_START_ADDRESS 0x3b13
...
//zhfan for control log pr458701
IMX135TRULYLogSwitch(kal_bool value)
{
...
static kal_bool IMX135TRULYOTP_Get_Module_Info()
{
...
#endif
//wenyuan.li@tcl 20130917 for module truly bug 	521820-    
if(KAL_FALSE == IMX135TRULY_ReadOtp(ModuleinforGroupBank[P1P2P3BankValidIndex],IMX135TRULYOTP_MDI_START_ADDRESS,MDIBufferVendor,IMX135TRULYOTP_MDI_SIZE))
    {
        JRDSENSORDB_ERROR("[liwenyuan]Read _Get_Module_Info error!\n");
        return -1;
    }
    VendorID = MDIBufferVendor[0];
    JRDSENSORDB("[liwenyuan]Module VendorID: %d\n", VendorID);
    return VendorID;
...
#endif
static kal_bool check_is_IMX135TRULY_otp_lsc_valid(void)
{
    kal_uint8 otpDataBuffer[IMX135TRULYOTP_LSC_FLAG_SIZE];
    kal_uint8 ret=KAL_FALSE;
    JRDSENSORDB("[liwenyuan]in %s line=%d\n",__func__,__LINE__);
    ret=IMX135TRULY_ReadOtp(IMX135TRULYOTP_LSC_FLAG_PAGE, IMX135TRULYOTP_LSC_FLAG_ADDRESS, &otpDataBuffer, IMX135TRULYOTP_LSC_FLAG_SIZE);
    if(ret!=KAL_TRUE)
    {
    ...
    {
        JRDSENSORDB_ERROR("[liwenyuan]in %s line=%d LSC FLAG is read 0x40!!\n",__func__,__LINE__);
        return KAL_TRUE;
    }
    else
    {
...
static kal_bool IMX135TRULYOTPShading1(void)//liwenyuan truly no compression
{
    ...
    kal_uint32 checksum = 0;
     ...
     JRDSENSORDB("[liwenyuan]in %s line=%d\n",__func__,__LINE__);
    //1. check is LSC valid
    ret = check_is_IMX135TRULY_otp_lsc_valid();
    ...
    //2. read shading from otp
    if(KAL_FALSE == IMX135TRULY_ReadOtp(IMX135TRULYOTP_LSC_START_PAGE,IMX135TRULYOTP_LSC_START_ADDRESS,&otpDataBuffer[0],IMX135TRULYOTP_LSC_SIZE))
    {
        JRDSENSORDB_ERROR("Read lsc OTP error!\n");
        return KAL_FALSE;
    }
    for(i = 0; i < IMX135TRULYOTP_LSC_SIZE; i++)
    {
        JRDSENSORDB("lscDataBuffer[%d] =0x%x\n", i, otpDataBuffer[i]);
        decompressData[i] = otpDataBuffer[i];
    }
...
UINT32 IMX135TRULYMIPIOpen(void)
{
    ...
    retry = 3; 
	SENSORDB("IMX135TRULYMIPIOpen enter:"); 
	kal_uint16 sensorid; 
    do {
...
UINT32 IMX135TRULYMIPIGetSensorID(UINT32 *sensorID) 
{
    ...
    kal_uint8  Vendor_ID = 0x0;
    // check if sensor ID correct
    SENSORDB("IMX135TRULYMIPIGetSensorID\n"); 
    do {		
	  // *sensorID =(kal_uint16)((IMX135TRULYMIPI_read_cmos_sensor(0x0000)<<8) | IMX135TRULYMIPI_read_cmos_sensor(0x0001)); 
	   ...
	   *sensorID = sensorIDH | sensorIDL;
        ...
        {
#ifdef IMX135TRULY_OTP
           otpRdStatus = IMX135TRULYOTPShading1(); //zhfan optimize otp pr458701
//[Begin]zhfan lens compatible:50003
           // Vendor_ID = IMX135TRULYOTPWbPregain(); //wenyuan.li read lensID
	//     SENSORDB("Vendor_ID =  0x%04x\n", Vendor_ID);
            Vendor_ID = IMX135TRULYOTP_Get_Module_Info(); //wenyuan.li read lensID
	     JRDSENSORDB_ERROR("Vendor_ID =  0x%04x\n", Vendor_ID);
	     Dump_OTP();//liwenyuan for test
	#if (defined(IMX135_LENS50016_MIPI_RAW)||defined(IMX135_MIPI_RAW)) 
            if(IMX135TRULY_ID != Vendor_ID)
            {
                *sensorID = 0;
	     ...
	     {
		    tcl_camera_id=IMX135_TRULY;
	     }
      #else
	  	    tcl_camera_id=IMX135_TRULY;
	#endif
#endif
		JRDSENSORDB_ERROR("cuirui tcl camera id = %d\n", tcl_camera_id);

            JRDSENSORDB_ERROR("Sensor ID = 0x%04x", *sensorID);
            break;
//[End]zhfan
        }
        SENSORDB("Read Sensor ID Fail = 0x%04x\n", *sensorID); 
...
//[end]pr470505 zhfan 2013-06-20
		...
		SENSORDB("[IMX135TRULYMIPI][Enter Fix_fps func] IMX135TRULYMIPI_Fix_Video_Frame_Rate = %d\n", u2FrameRate/10);
                                                                ...
                                                                UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_LOG_SWITCH:
            IMX135TRULYLogSwitch((kal_bool)*pFeatureData16); //zhfan for control log pr458701
        default:
            break;
    }
    return ERROR_NONE;
