...
#define SENSORDB(fmt, arg...) printk( "[IMX135MIPIRaw_Truly] "  fmt, ##arg)
#define RETAILMSG(x,...)
#define TEXT
...
static kal_uint8 IMX135TrulyOTPReadSensor(kal_uint16 address)
{
    ...
    SENSORDB("!!!!!address=0x%x, reVal= 0x%x!!!!!\n", address, reVal);
    return reVal;
}
...
bool start_read_otp(BYTE zone)
{
	BYTE val = 0;
	int i;
	IMX135TrulyOTPWriteSensor(0x3B02, zone);   //PAGE
	IMX135TrulyOTPWriteSensor(0x3B00, 0x01);
	Sleep(5);
	for(i=0;i<100;i++)
	{
		val = IMX135TrulyOTPReadSensor(0x3B01);
		if((val & 0x01) == 0x01)
			break;
		Sleep(2);
	}
...
BYTE get_otp_flag(BYTE zone)
{
	...
	flag = IMX135TrulyOTPReadSensor(0x3B04);
	flag = flag & 0xc0;
...
bool get_otp_date(BYTE zone) 
{
	...
	year  = IMX135TrulyOTPReadSensor(0x3B06);
	month = IMX135TrulyOTPReadSensor(0x3B07);
	day   = IMX135TrulyOTPReadSensor(0x3B08);
...
BYTE get_otp_module_id(BYTE zone)
{
	...
	module_id = IMX135TrulyOTPReadSensor(0x3B05);
...
BYTE get_otp_lens_id(BYTE zone)
{
	...
	lens_id = IMX135TrulyOTPReadSensor(0x3B09);
...
BYTE get_otp_vcm_id(BYTE zone)
{
	...
	vcm_id = IMX135TrulyOTPReadSensor(0x3B0A);
...
BYTE get_otp_driver_id(BYTE zone)
{
	...
	driver_id = IMX135TrulyOTPReadSensor(0x3B0B);
...
BYTE get_light_id(BYTE zone)
{
	...
	light_id = IMX135TrulyOTPReadSensor(0x3B0C);
...
BYTE get_lsc_flag()
{
	...
	flag = IMX135TrulyOTPReadSensor(0x3B43);
	flag = flag & 0xc0;
	printk("Flag:0x%02x",flag );
	return flag;
}
...
bool otp_lenc_update()
{
	...
	for(i=0;i<8;i++)
	{
		...
		for(j=0;j<64;j++)
     		lsc_data[i*64+j] = IMX135TrulyOTPReadSensor(0x3B04+j);
	}

	for(i=0;i<504;i++) //LSC SIZE is 504 BYTES
		IMX135TrulyOTPWriteSensor(0x4800+i, lsc_data[i]);

	//Enable LSC
	temp1 = IMX135TrulyOTPReadSensor(0x0700);
	temp2 = IMX135TrulyOTPReadSensor(0x3A63);
	temp1 = temp1 | 0x01;
	temp2 = temp2 | 0x01;
	IMX135TrulyOTPWriteSensor(0x0700, temp1);
	IMX135TrulyOTPWriteSensor(0x3A63, temp2);
...
bool get_otp_wb(BYTE zone)
{
	...
	temph = IMX135TrulyOTPReadSensor(0x3B18);  
	templ = IMX135TrulyOTPReadSensor(0x3B19);   
	golden_r  = (USHORT)templ + (((USHORT)temph & 0x03) << 8);

	temph = IMX135TrulyOTPReadSensor(0x3B1A);
	templ = IMX135TrulyOTPReadSensor(0x3B1B);
	golden_gr  = (USHORT)templ + (((USHORT)temph & 0x03) << 8);

	temph = IMX135TrulyOTPReadSensor(0x3B1C);
	templ = IMX135TrulyOTPReadSensor(0x3B1D);
	golden_gb  = (USHORT)templ + (((USHORT)temph & 0x03) << 8);

	temph = IMX135TrulyOTPReadSensor(0x3B1E);
	templ = IMX135TrulyOTPReadSensor(0x3B1F);
	golden_b  = (USHORT)templ + (((USHORT)temph & 0x03) << 8);

	temph = IMX135TrulyOTPReadSensor(0x3B10);  
	templ = IMX135TrulyOTPReadSensor(0x3B11);   
	current_r  = (USHORT)templ + (((USHORT)temph & 0x03) << 8);

	temph = IMX135TrulyOTPReadSensor(0x3B12);
	templ = IMX135TrulyOTPReadSensor(0x3B13);
	current_gr  = (USHORT)templ + (((USHORT)temph & 0x03) << 8);

	temph = IMX135TrulyOTPReadSensor(0x3B14);
	templ = IMX135TrulyOTPReadSensor(0x3B15);
	current_gb  = (USHORT)templ + (((USHORT)temph & 0x03) << 8);

	temph = IMX135TrulyOTPReadSensor(0x3B16);
	templ = IMX135TrulyOTPReadSensor(0x3B17);
...
#ifdef IMX135TRULY_OTP
SENSORDB("IMX135MIPI_Sensor_Init truly otp enter\n");
	otp_update();
SENSORDB("IMX135MIPI_Sensor_Init truly otp end\n");
#endif
