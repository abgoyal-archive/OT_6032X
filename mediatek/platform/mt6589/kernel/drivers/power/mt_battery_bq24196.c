...
#define TOPOFF_CHG_VOLTAGE      4300
int g_battery_boot_count=0;
int g_battery_boot_count_once=0;
...
#if defined(SCRIBEPRO_WIRELESS_CHARGING)
int g_switch_wireless_wire = 1;
#endif
#define JRD_CHARGING_NOT_FULL_VOLTAGE      4310
...
void scover_otg_callback(int attached)
{
	if(1==attached)
	{
		 Cover_Exist=1;
		 ...
		 mt_set_gpio_out(GPIO75,GPIO_OUT_ONE);
	}
	else
	{
		Cover_Exist=0;
		if(0==OTG_Exist)
		{
			mt_set_gpio_mode(GPIO75,GPIO_MODE_GPIO);
			mt_set_gpio_dir(GPIO75,GPIO_DIR_OUT);
			mt_set_gpio_out(GPIO75,GPIO_OUT_ZERO);
			mt_set_gpio_mode(GPIO79,GPIO_MODE_GPIO);
			mt_set_gpio_dir(GPIO79,GPIO_DIR_OUT);
			mt_set_gpio_out(GPIO79,GPIO_OUT_ZERO);
		}
...
kal_uint32 DEFAULT_SYNC_TIME_OUT=300; //5mins
...
#if defined(SCRIBEPRO_CHARGING_TEMPERATURE_PLOICY)
    ...
    else
    {
        if ( BMT_status.charger_type == STANDARD_HOST )
        {
            ...
            else
            {
                ...
                bq24196_set_iinlim(0x2); //IN current limit at 0.5A		  
                bq24196_set_ac_current();
                if (Enable_BATDRV_LOG == 1) {
                    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY:bq24196] bq24196_set_ac_current(), CC value(%d) \r\n", g_temp_CC_value);
                }
        ...
        else if (BMT_status.charger_type == NONSTANDARD_CHARGER)
        {
            ...
            bq24196_set_iinlim(0x7); //IN current limit at 3A			
            bq24196_set_ac_current();
        }
        else if (BMT_status.charger_type == STANDARD_CHARGER)
        {
...
void ChargerHwInit_bq24196(void)
{
    ...
    //pull CE low
    mt_set_gpio_mode(GPIO22 + GPIO_EXTEND_START,GPIO_MODE_GPIO);  
    mt_set_gpio_dir(GPIO22 + GPIO_EXTEND_START,GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO22 + GPIO_EXTEND_START,GPIO_OUT_ZERO);       
    bq24196_set_en_hiz(0x0);
...
void pchr_turn_on_charging_bq24196 (void)
{
    ...
    else
    {
    	...
    	if (Enable_BATDRV_LOG == 1)
	{
			printk("[BATTERY:bq24196] [pchr_turn_on_charging_bq24196] Select charging charger wireless or wire g_switch_wireless_wire=%d\n",g_switch_wireless_wire);
	}	
    	if(g_switch_wireless_wire==1)
    	{
		g_temp_CC_value = 500;
            bq24196_set_iinlim(0x2); //IN current limit at 500mA
            bq24196_set_ac_current();
		if (Enable_BATDRV_LOG == 1)
		{
			printk("[BATTERY:bq24196] [pchr_turn_on_charging_bq24196] Select wireless charging current! \n");
		}
    	}
	else
	{
		...
		if (Enable_BATDRV_LOG == 1)
		{
			printk("[BATTERY:bq24196] [pchr_turn_on_charging_bq24196] Select wire charging current!\n");
		}
	}    				
	#else
		select_charging_curret_bq24196();
...
int BAT_CheckBatteryStatus_bq24196(void)
{
    ...
    {
        if(BMT_status.temperature <=(CHARGE_TEMPERATURE_STEP2-1))
        {
...
//            FGADC_Reset_SW_Parameter();
    ...
    if( upmu_is_chr_det() == KAL_TRUE)
    {
           if((BMT_status.SOC == 99)&&(BMT_status.bat_vol >= RE_TIMER_VOLTAGE))
           {
	...
	if((BMT_status.SOC <= 95)&& (BMT_status.bat_full) && (g_HW_Charging_Done == 1) && (!g_Battery_Fail))
	{
		 BMT_status.bat_full = KAL_FALSE;
            g_bat_full_user_view = KAL_TRUE;
            //BMT_status.bat_charging_state = CHR_PRE;
            ...
            g_HW_Charging_Done = 0;
            g_Calibration_FG = 0;
		printk("JRD recovery charging <95% ,BMT_status.bat_vol=%d\n",BMT_status.bat_vol);
	}
     }
    return PMU_STATUS_OK;
}
...
void BAT_thread_bq24196(void)
{
    ...
    int returnData = 0;
#endif	
	if(g_battery_boot_count_once == 0)
	{
		g_battery_boot_count ++;
		if(g_battery_boot_count ==12)
		{
			gFG_15_vlot+= 15;
			g_battery_boot_count_once = 1; 
		}
	}
#if defined(SCRIBEPRO_WIRELESS_CHARGING)
    g_switch_wireless_wire= mt_get_gpio_in(GPIO73);    
    if (Enable_BATDRV_LOG == 1) 
     {		
	printk("[BATTERY:bq24196] [BAT_thread_bq24196] g_switch_wireless_wire=%d\n",g_switch_wireless_wire);
     }
    ...
    if( upmu_is_chr_det() == KAL_TRUE )
    {
		...
		{
			CHR_Type_num=4;
			xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BAT_thread:bq24196] wireless charging CHR_Type_num = %d\r\n", CHR_Type_num);
	...
	if((BMT_status.TOPOFF_charging_time > 15*60) && (BMT_status.bat_charging_state == CHR_TOP_OFF))
	{
	     BMT_status.bat_full = KAL_FALSE;
            g_bat_full_user_view = KAL_TRUE;
            g_HW_Charging_Done = 0;
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY:bq24196] Battery sync UI full from 99 to 100%.\n");
	}
        bq24196_stat = bq24196_get_chrg_stat();

        /* check battery full */
        if((BMT_status.bat_vol >= JRD_CHARGING_NOT_FULL_VOLTAGE) && (BMT_status.bat_vol > RECHARGING_VOLTAGE) && (bq24196_stat == 0x3))
        {
...
void PrechargeCheckStatus(void)
{
    ...
    if (Enable_BATDRV_LOG == 1) 
    {		
	printk("[BATTERY:bq24196] [PrechargeCheckStatus] g_switch_wireless_wire=%d\n",g_switch_wireless_wire);
    }
#endif
    if(g_FG_init == 1)
        return;
...
//			if(upmu_get_cid() == 0x1020)
      if(0)
      	...
      	if (Enable_BATDRV_LOG == 1)
	{
		printk("[BATTERY:bq24196] [PrechargeCheckStatus] Select charging charger wireless or wire g_switch_wireless_wire=%d\n",g_switch_wireless_wire);
	}	
    	if(g_switch_wireless_wire==1)
    	{
		g_temp_CC_value = 500;
		...
		if (Enable_BATDRV_LOG == 1)
		{
			printk("[BATTERY:bq24196] [PrechargeCheckStatus] Select wireless charging current! \n");
		}
    	}
	else
	{				
		...
		if (Enable_BATDRV_LOG == 1)
		{
			printk("[BATTERY:bq24196] [PrechargeCheckStatus] Select wire charging current!\n");
		}
	}
#else
	select_charging_curret_bq24196();
            ...
            else
            {
                ...
                bat_data->BAT_TECHNOLOGY = POWER_SUPPLY_TECHNOLOGY_LION;
                bat_data->BAT_HEALTH = POWER_SUPPLY_HEALTH_GOOD;
                bat_data->BAT_batt_vol = 3600;
                bat_data->BAT_batt_temp = 10 * bat_temp;
                bat_data->BAT_PRESENT = 1;
                bat_data->BAT_CAPACITY = 10;
                bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_CHARGING;
...
void battery_kthread_hrtimer_init(void)
{
    ...
    ktime = ktime_set(6, 0);	// 7s, 5* 1000 ms
...
static int mt6320_battery_probe(struct platform_device *dev)    
{
	...
	mt_set_gpio_pull_enable(GPIO73,GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO73,GPIO_PULL_UP);
#endif
    xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "******** MT6320 battery driver probe!! ********\n" );
