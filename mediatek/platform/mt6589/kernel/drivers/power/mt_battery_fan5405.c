...
#if defined DIABLOPHA_FG_15_TRACK
int FG_15_vlot_counter = 0;
int FG_15_vlot_flag = 0;
int gFG_15_vlot_origianl = 0;
int FG_15_TRACK_TIME = 0; // less than 8 Min remain 15%
#endif
////////////////////////////////////////////////////////////////////////////////
// JEITA 
////////////////////////////////////////////////////////////////////////////////
...
#endif
#if defined(JRD_SCRIBE5HD_BORAD)
...
#if defined(DIABLOPHA_CHARGING_TEMPERATURE_PLOICY)
...
#if (defined (DIABLOX_BATTERY_ID_CHECK) || defined (DIABLOPHA_BATTERY_ID_CHECK))
#define BATT_ID_CHECK_SUPPORT 1
#endif

#ifdef BATT_ID_CHECK_SUPPORT
#define AUXADC_BATT_ID_CHANNEL 4
#if defined(DIABLOPHA_BATTERY_ID_CHECK)
#define CHG_MIN_BYD_BATT_ID  400
#define CHG_MAX_BYD_BATT_ID  600
#define CHG_MIN_SCUD_BATT_ID  1300
#define CHG_MAX_SCUD_BATT_ID  1500
#elif defined (DIABLOX_BATTERY_ID_CHECK)
#define CHG_MIN_BYD_BATT_ID  540
#define CHG_MAX_BYD_BATT_ID  740
#define CHG_MIN_SCUD_BATT_ID  950
#define CHG_MAX_SCUD_BATT_ID  1400
#define ID_BUFFER_SIZE 12
kal_bool id_buffuer_first = KAL_TRUE;
static int id_buffer_index;
static int batteryIDSum = 0;
static unsigned short batteryIDBuffer[ID_BUFFER_SIZE];
#endif
int byd_sucd_batt_id;// 1 BYD 2 SUCD
extern int IMM_GetOneChannelValue(int dwChannel, int data[4], int* rawdata);
void check_battery_id(void);
#endif
#define TOPOFF_CHG_VOLTAGE      4250
///////////////////////////////////////////////////////////////////////////////////////////
...
void check_battery_id(void){
   ...
   int rawdata=0;
   long tmo1;
#ifdef DIABLOX_BATTERY_ID_CHECK
   int bat_temperature_volt=0;
 
   int fg_r_value=0;
   kal_int32 fg_current_temp=0, fg_current_temp1=0;
   kal_bool fg_current_state, fg_current_state1;
   int bat_id_volt_temp=0;
   
  fg_current_temp1 = fgauge_read_current();
  fg_current_state1 = get_gFG_Is_Charging();
#endif
  ...
  //printk("%s: bat_id_vol:%d!\n", __func__, bat_id_vol);
	    ...
	    if(!id_buffuer_first)
	    {
	    // for the first time battery id check must have a battery id so it can't return here
	      if((fg_current_temp - fg_current_temp1 > 2000)||(fg_current_state1!=fg_current_state))
	      	{
	      	 xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "check_battery_id%d,%d,%d,%d,%d\n", 
              bat_id_vol, fg_current_temp, fg_current_temp1, fg_current_state,fg_current_state1);  
                return;
		}
	     }
	      fg_current_temp = fg_current_temp/10;
		bat_id_volt_temp = bat_id_vol;
            if(fg_current_state==KAL_TRUE)
            {
                bat_id_vol = bat_id_vol - ((fg_current_temp*fg_r_value)/1000);
            }
            else
            {
                bat_id_vol = bat_id_vol + ((fg_current_temp*fg_r_value)/1000);
	...
	if(id_buffuer_first)
  	{
      	    ...
      	    {
           batteryIDBuffer[i] = bat_id_vol;
      	    }
	    batteryIDSum = bat_id_vol*ID_BUFFER_SIZE;
	    id_buffuer_first = KAL_FALSE;
      }
	else
      {
	   batteryIDSum -= batteryIDBuffer[id_buffer_index];
	   batteryIDSum +=bat_id_vol;
	   batteryIDBuffer[id_buffer_index] = bat_id_vol;
    	   id_buffer_index ++;
    	   if(id_buffer_index >= ID_BUFFER_SIZE)
         id_buffer_index = 0;
      }
	bat_id_vol = batteryIDSum/ID_BUFFER_SIZE;
      xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "check_battery_id%d,%d,%d,%d,%d\n", 
              bat_id_vol, fg_current_state, fg_current_temp, fg_r_value,bat_id_volt_temp);   
#endif
...
static void mt6320_battery_update(struct mt6320_battery_data *bat_data)
{
    ...
    {
	#if defined DIABLOPHA_FG_15_TRACK
	if (BMT_status.SOC > 15)
	{
		if(FG_15_vlot_flag == 1)
		{
			gFG_15_vlot -= 30; // recover origianl value when the voltage gFG_voltage < gFG_15_vlot_origianl
			FG_15_vlot_counter = 0;
			FG_15_vlot_flag = 0;
			FG_15_vlot_counter = 0;
		}
	}
	...
	#if defined DIABLOPHA_FG_15_TRACK
	if (gFG_voltage < gFG_15_vlot_origianl)
	{
		if(FG_15_vlot_flag == 1)
		{
			gFG_15_vlot -= 30; // recover origianl value when the voltage gFG_voltage < gFG_15_vlot_origianl
	...
	#if defined DIABLOPHA_FG_15_TRACK
		FG_15_vlot_counter++;
		if(FG_15_vlot_counter > FG_15_TRACK_TIME) // less than 8 Min remain 15%
		{
			if(FG_15_vlot_flag == 0)
			{
				gFG_15_vlot_origianl = gFG_15_vlot;
				gFG_15_vlot += 30; // add the 30mV voltage in order to low percent is decrease slowly
				FG_15_vlot_flag = 1;
			}
		}
	#endif
...
void ChargerHwInit_fan5405(void)
{
    ...
    if(temp_init_flag==0)
    {
        ...
        fan5405_config_interface_liao(0x01,0xf9);
		#else
	...
	((BMT_status.temperature >= CHARGE_TEMPERATURE_STEP2)&&(charge_temperature_above_step3==1)))
       fan5405_config_interface_liao(0x02,0x7a); //VREG 4.1V
    else
       fan5405_config_interface_liao(0x02,0xaa); //VREG 4.34V
#else
   	 fan5405_config_interface_liao(0x02,0xaa); //VREG 4.34V
#endif                
#endif
...
#endif
#else
            fan5405_config_interface_liao(0x04,0x6B); //194mA
#endif
        temp_init_flag =1;    
    }
    else
...
if(BMT_status.temperature < 38)
 ...
 else if (BMT_status.temperature < 41)
 {
 reg_set_value=0x40;
 }
 else if (BMT_status.temperature < 43)
 {
 reg_set_value=0x30;
 }
 else
...
#else
		fan5405_config_interface_liao(0x04,0x09);
#endif
#endif                
                if (Enable_BATDRV_LOG == 1) {
...
void pchr_turn_on_charging_fan5405(void)
{
    ...
    else if( ((get_boot_mode()==META_BOOT) || (get_boot_mode()==ADVMETA_BOOT))&&(upmu_get_rgs_baton_undet() ==1) )
    {   
...
int BAT_CheckBatteryStatus_fan5405(void)
{
    ...
    {
        if( (BMT_status.temperature <= HIGH_TEMPERATURE_RECOVER) && (BMT_status.temperature >= LOW_TEMPERATURE_RECOVER) )
        {
...
{    
	...
	if((BMT_status.TOPOFF_charging_time > 15*60) && (BMT_status.bat_charging_state == CHR_TOP_OFF))
        {
	     ...
	     g_bat_full_user_view = KAL_TRUE;
            ...
            BMT_status.POSTFULL_charging_time = 0;
            g_HW_Charging_Done = 0;
            printk(KERN_INFO "jrd %s: Battery full and sync UI show to 100% \n", __func__);
            return;
	...
	fan5405_config_interface_liao(0x04,0x6b);
...
#if defined(DIABLOPHA_BATTERY_ID_CHECK)
	mt_set_gpio_mode(117,GPIO_MODE_GPIO);
	mt_set_gpio_dir(117,GPIO_DIR_IN);
	mt_set_gpio_pull_enable(117,GPIO_PULL_DISABLE);
	msleep(60);
#endif
