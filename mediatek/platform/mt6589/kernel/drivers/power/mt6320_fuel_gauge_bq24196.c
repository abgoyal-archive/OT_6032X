...
*******************************************************************************/
kal_int32 oam_v_ocv_for_test=0;
...
*
*******************************************************************************/
#define FG_VBAT_AVERAGE_SIZE 18 // 36*5s=180s=3mins
...
//4. update DOD0 after booting Xs
    if(gFG_booting_counter_I_FLAG == 1)
    {
        ...
        {
            if( ( g_rtc_fg_soc != 0                     ) &&                    
                ( (g_rtc_fg_soc-gFG_capacity_by_v) < 40 ) &&
        ...
        {
            if( ( g_rtc_fg_soc != 0                     ) &&                    
                ( (gFG_capacity_by_v-g_rtc_fg_soc) < 40 ) &&
                (( gFG_capacity_by_v > 5 || upmu_is_chr_det() == KAL_TRUE ) ||
                (g_boot_reason == BR_WDT_BY_PASS_PWK)) 
                )
            {
	...
	if( (gFG_15_vlot > 3800) || (gFG_15_vlot < 3600) ) 
        ...
        #endif        
		 //gFG_15_vlot+= 25;
		 //compensate 25mv for gFG_15_vlot
...
// 2. SW algorithm initialization
    //gFG_voltage = fgauge_read_voltage();            
    gFG_voltage = get_hw_ocv();
    oam_v_ocv_for_test = gFG_voltage;
    gFG_current = fgauge_read_current();   
    i=0;
    while( gFG_current == 0 )
    {
...
{    
    ...
    //if (Enable_FGADC_LOG >= 1) 
    //{    
        xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\r\n",
            ...
            CAR_TUNE_VALUE,AGING_TUNING_VALUE,oam_v_ocv_for_test);            
    //}
