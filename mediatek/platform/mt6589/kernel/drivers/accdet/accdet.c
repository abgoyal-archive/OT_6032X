...
void accdet_work_callback(struct work_struct *work)
{
	...
	/* Avoid cable type to mistake detection with headphone when headset slowly plugging in, sleep 500ms and check cable type again */
	if (cable_type == HEADSET_NO_MIC) {
		msleep(1200);
	  	check_cable_type();
	}
...
static int accdet_probe(struct platform_device *dev)	
{
	...
	{	
       ...
       #endif
#if defined(JRD_IPHONE_HDST_COMPATIBILITY) || defined(JRD_DIABLOALPHA)
...
static int accdet_pm_restore_noirq(struct device *device)
{
	...
	switch_set_state((struct switch_dev *)&accdet_data, cable_type);
#ifdef JRD_DIABLOALPHA
        /*For diabloalpha, eint_accdet_sync_flag must always be 1 and accdet must always be on.*/
	if (0) {
#else
	if (cable_type == NO_DEVICE) {
#endif
