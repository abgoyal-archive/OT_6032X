			...
			}else
			{
				if(level == 0)
				{
				       if(!strcmp(cust->name,"red"))
						{
#if defined(RAV4_LED)||defined(DIABLOX_LED)||defined(SCRIBEPRO_LED)

							mt_set_gpio_out(GPIO28,GPIO_OUT_ZERO);//for power saving
							LEDS_DEBUG("[LED]      GPIO28\n");
#elif defined(SCRIBEPRO_TD_LED)

							mt_set_gpio_out(GPIO161,GPIO_OUT_ZERO);
							LEDS_DEBUG("[LED]      GPIO161\n");
#elif defined(DIABLOX_TD_LED)
							mt_set_gpio_out(GPIO25 + GPIO_EXTEND_START,GPIO_OUT_ZERO);
							LEDS_DEBUG("[LED]      GPIO25 + GPIO_EXTEND_START\n");
#endif
						}
				      ...
				      if(!strcmp(cust->name,"red"))
						{
							...
							mt_set_gpio_out(GPIO161,GPIO_OUT_ONE);
							LEDS_DEBUG("[LED]      GPIO161\n");
#elif defined(DIABLOX_TD_LED)
							mt_set_gpio_out(GPIO25 + GPIO_EXTEND_START,GPIO_OUT_ONE);
							LEDS_DEBUG("[LED]      GPIO25 + GPIO_EXTEND_START\n");
#endif
...
static int __init mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
...
#elif defined(DIABLOX_LED)||defined(SCRIBEPRO_LED)
		g_leds_data[i]->cust.data = PWM7;
		mt_set_gpio_mode(GPIO75, GPIO_MODE_01);
		mt_set_gpio_mode(GPIO28, GPIO_MODE_00);
		mt_set_gpio_dir(GPIO28, GPIO_DIR_OUT);
		...
		g_leds_data[i]->cust.data = PWM7;
		mt_set_gpio_mode(GPIO75, GPIO_MODE_01);
		mt_set_gpio_mode(GPIO161, GPIO_MODE_00);
		mt_set_gpio_dir(GPIO161, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO161, GPIO_OUT_ZERO);
		mt65xx_led_set_cust(&cust_led_list[0],0);
#elif defined(DIABLOX_TD_LED)
		g_leds_data[i]->cust.data = PWM7;
		mt_set_gpio_mode(GPIO75, GPIO_MODE_01);
		mt_set_gpio_mode(GPIO25 + GPIO_EXTEND_START, GPIO_MODE_00);
		mt_set_gpio_dir(GPIO25 + GPIO_EXTEND_START, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO25 + GPIO_EXTEND_START, GPIO_OUT_ZERO);
		mt65xx_led_set_cust(&cust_led_list[0],0);
#endif
