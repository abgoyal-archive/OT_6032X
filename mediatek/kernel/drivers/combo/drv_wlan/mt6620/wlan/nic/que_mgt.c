    ...
    )
{
    ...
    do{
        ...
        if (!HIF_RX_HDR_GET_80211_FLAG(prHifRxHdr)){
            ...
            // PR438104 BEGIN---The connected AP often disconnected automaticly---QIHONG.XU@TCL.COM---Jun 10,2013
            //if (IS_BMCAST_MAC_ADDR(pucEthDestAddr)) {
            if (IS_BMCAST_MAC_ADDR(pucEthDestAddr) && (OP_MODE_ACCESS_POINT != prBssInfo->eCurrentOPMode)) {
            // PR438104 END---The connected AP often disconnected automaticly---QIHONG.XU@TCL.COM---Jun 10,2013
                fgIsBMC = TRUE;
            }
