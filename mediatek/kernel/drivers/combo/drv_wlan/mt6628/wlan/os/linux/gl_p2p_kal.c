...
** $Log: gl_p2p_kal.c $
**
 * 11 06 2013 qihong.xu
 * [540293] [MT6628 Wi-Fi] [Driver] Fix Wi-Fi auto connect fail after unlock
**
*/
    ...
    )
{
    ...
    for(i = 0; i < 8; i++) {
        if(UNEQUAL_MAC_ADDR(rbssid, aucNullAddr)){
            if(EQUAL_MAC_ADDR(&(prGlueInfo->prP2PInfo->aucblackMACList[i]), rbssid)) {
                fgIsExsit = TRUE;
                //PR540293 BEGIN---Fix auto connect fail after unlock---QIHONG.XU@TCL.COM---Nov 06,2013
                printk("It is a blocked station!\n");
                //PR540293 END---Fix auto connect fail after unlock---QIHONG.XU@TCL.COM---Nov 06,2013
                return fgIsExsit;
            }
        }
    }
