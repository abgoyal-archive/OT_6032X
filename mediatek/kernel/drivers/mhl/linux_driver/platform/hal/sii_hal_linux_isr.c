...
#include <cust_eint.h>
#define GPIO_MHL_EINT_PIN (GPIOEXT23)
...
halReturn_t HalInstallIrqHandler(fwIrqHandler_t irqHandler)
{
	...
	/*mtk eint setup*/
	mt_set_gpio_mode(GPIO_MHL_EINT_PIN, GPIO_MODE_02);
	mt_set_gpio_dir(GPIO_MHL_EINT_PIN, GPIO_DIR_IN);
	mt_set_gpio_pull_select(GPIO_MHL_EINT_PIN, GPIO_PULL_UP);
	mt_set_gpio_pull_enable(GPIO_MHL_EINT_PIN, GPIO_PULL_ENABLE);

	mt65xx_eint_mask(CUST_EINT_MHL_NUM);
	mt65xx_eint_set_sens(CUST_EINT_MHL_NUM, CUST_EINT_LEVEL_SENSITIVE);
	mt65xx_eint_set_hw_debounce(CUST_EINT_MHL_NUM, 256);
	mt65xx_eint_registration(CUST_EINT_MHL_NUM, CUST_EINT_DEBOUNCE_ENABLE,
                             CUST_EINT_POLARITY_LOW, &mhl8338_irq_handler, 0);
	mt65xx_eint_unmask(CUST_EINT_MHL_NUM);
