...
#define CUST_EINT_MHL_NUM (209)
#define GPIO_MHL_5V_PIN (GPIOEXT28)
#define GPIO_MHL_MACRO_PROJECTOR_PWR_PIN (GPIO18)
...
extern void SiiMhlTxDrvTmdsControl (bool_t);
static int hdmi_drv_video_enable(bool enable)
{
    SiiMhlTxDrvTmdsControl(enable);
    return 0;
}
...
#define GPIO_MHL_RST_B_PIN (GPIOEXT24)
...
int hdmi_drv_power_on(void)
{
	...
	/*1.2v pmic input for mhl*/
	hwPowerOn(MT65XX_POWER_LDO_VAST, VOL_1200, "mhl_ldo_vast");

	/*5v power supply*/
	mt_set_gpio_out(GPIO_MHL_5V_PIN, GPIO_OUT_ONE);

    /*macro projector power supply*/
    mt_set_gpio_mode(GPIO_MHL_MACRO_PROJECTOR_PWR_PIN, GPIO_MODE_00);
	mt_set_gpio_dir(GPIO_MHL_MACRO_PROJECTOR_PWR_PIN, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO_MHL_MACRO_PROJECTOR_PWR_PIN, GPIO_OUT_ONE);

	/*reset*/
	mt_set_gpio_mode(GPIO_MHL_RST_B_PIN, GPIO_MODE_00);
	mt_set_gpio_dir(GPIO_MHL_RST_B_PIN, GPIO_DIR_OUT);
	mt_set_gpio_pull_enable(GPIO_MHL_RST_B_PIN, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_MHL_RST_B_PIN, GPIO_PULL_UP);
	SiiMhlTxHwReset(5,5);
...
void hdmi_drv_power_off(void)
{
	...
	/*internal power shutdown*/
	SiiMhlTxHwGpioSuspend();
	SiiMhlTxHwReset(50,200);
	ForceSwitchToD3();

	/*shutdown bb power supply*/
    mt_set_gpio_out(GPIO_MHL_MACRO_PROJECTOR_PWR_PIN, GPIO_OUT_ZERO);
	mt_set_gpio_out(GPIO_MHL_5V_PIN, GPIO_OUT_ZERO);
	hwPowerDown(MT65XX_POWER_LDO_VAST, "mhl_ldo_vast");
}
