...
static long hdmi_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
       ...
       case MTK_HDMI_AUDIO_VIDEO_ENABLE:
       {
        ...
        {
            /*
             * JRD: video/audio enable/disable trigger as UE design
             * [FIXME] it's better to add JRD_HDMI_AV_ENABLE case
             */
            printk("[hdmitx] ioctl HDMI_VIDEO_ENABLE, with arg %d\n", arg);
            if(arg) {
                hdmi_video_enable(true);
            } else {
                hdmi_video_enable(false);
            }
