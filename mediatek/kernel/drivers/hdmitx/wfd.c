...
#define WFD_DISPLAY_ZOOM_OUT_FACTOR 5
		...
		case MTK_EXT_DISPLAY_ENTER:
		{
			...
			if(p->is_enabled)
				return 0;
                        ...
                        //add by fang.song for PR463586 begin
                        if(IS_HDMI_ON())
                        {
                            SET_HDMI_OFF();
                            HDMI_CHECK_RET(hdmi_drv_deinit());
                            WFD_LOG("Display Memory error: overlay_dst_va and ddp_dst_va will be freed\n");
                        ...
                        //add by fang.song for PR463586 end
