...
static int __sock_diag_rcv_msg(struct sk_buff *skb, struct nlmsghdr *nlh)
{
	...
	if (nlmsg_len(nlh) < sizeof(*req))
		return -EINVAL;
        /*porting P153_JRD497347_For_JRDSZ89_WE_JB2_ALPS.JB2.MP.V1.9_P153_.tar.gz */
	/* zlliu add for PR491385 begin
	 * Userland can send a netlink message requesting SOCK_DIAG_BY_FAMILY with a
	 * family greater or equal then AF_MAX -- the array size of sock_diag_handlers[].
	 * The current code does not test for this condition therefore is vulnerable to an
	 * out-of-bound access opening doors for a privilege escalation.
	 */
	if (req->sdiag_family >= AF_MAX)
		return -EINVAL;
	/* zlliu add for PR491385 end*/
        /*porting P153_JRD497347_For_JRDSZ89_WE_JB2_ALPS.JB2.MP.V1.9_P153_.tar.gz*/
