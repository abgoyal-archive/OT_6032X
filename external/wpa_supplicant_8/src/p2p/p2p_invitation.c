				...
				const u8 *data, size_t len, int rx_freq)
{
	...
	if (p2p_parse(data, len, &msg))
		return;
//PR544851-Qiao.Li-001 Added begin
#if 1
        struct wpa_bss *bss = NULL;
	struct wpa_supplicant *wpa_s = p2p->cfg->cb_ctx;
	int persistent_group = 0;
	bss = wpa_bss_get_bssid(wpa_s, msg.group_bssid);
	dev = p2p_get_device(p2p, sa);
	wpa_printf(MSG_DEBUG, "0x%x, 0x%x, 0x%x", bss, msg.group_info, dev);
	if (msg.invitation_flags)
		persistent_group = *msg.invitation_flags & P2P_INVITATION_FLAGS_TYPE;
	else {
                wpa_printf(MSG_DEBUG, "P2P: Mandatory Invitation Flags "
			   "attribute missing from Invitation Request");
		persistent_group = 1;
	}
	if ((bss && dev && (!persistent_group)
		&& !wpa_s->current_ssid) ||
                (bss && dev && (!persistent_group)
		&& wpa_s->current_ssid
		&& (wpa_s->current_ssid->mode != WPAS_MODE_P2P_GO))) {
		bss->freq = p2p_channel_to_freq((const char *)
						     msg.operating_channel,
						     msg.operating_channel[3],
						     msg.operating_channel[4]);
		dev->oper_freq = bss->freq;
		wpa_printf(MSG_DEBUG, "MTK %s, %d, freq: %dMHz", __func__, __LINE__, bss->freq);
	}
#endif
//PR544851-Qiao.Li-001 Added end
...
//PR544851-Qiao.Li-002 Added begin
#if 1
	struct wpa_bss *bss = NULL;
	struct wpa_supplicant *wpa_s = p2p->cfg->cb_ctx;
	int persistent_group = 0;
	bss = wpa_bss_get_bssid(wpa_s, sa);
	if (msg.invitation_flags)
		persistent_group = *msg.invitation_flags & P2P_INVITATION_FLAGS_TYPE;
	else {
		wpa_printf(MSG_DEBUG, "P2P: Mandatory Invitation Flags "
...
#endif
//PR544851-Qiao.Li-002 Added end
