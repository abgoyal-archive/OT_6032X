...
/// M: visible image process & improve gif animation performance
#include "IntRect.h"
#endif
...
#if ENABLE(VISIBLE_IMAGE_MANAGEMENT)
/// M: visible image processing & improve gif animation performance @{
public:
    void visibleImageProcess(const IntRect& visibleScreenRect);
    HashSet<CachedResourceLoader*> m_cachedResourceLoaders;
    void triggerAshmemMonitor();
    void unTriggerAshmemMonitor();
    bool hasTriggerAshmemMonitor();
    private:
       int volatile m_ashmemMonTriggerFlag;
/// @}
#endif
};
