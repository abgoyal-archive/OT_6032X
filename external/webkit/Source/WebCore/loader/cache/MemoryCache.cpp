...
void MemoryCache::visibleImageProcess(const IntRect& visibleScreenRect)
{
    ...
    for (HashSet<CachedResourceLoader*>::iterator itr = m_cachedResourceLoaders.begin(); itr != end; ++itr) {
        ...
        (*itr)->visibleImageProcess(visibleScreenRect);
    }
    if (hasTriggerAshmemMonitor())
       unTriggerAshmemMonitor();
}
...
void MemoryCache::triggerAshmemMonitor()
{
  WTF::atomicIncrement(&m_ashmemMonTriggerFlag);
}
void MemoryCache::unTriggerAshmemMonitor()
{
  WTF::atomicDecrement(&m_ashmemMonTriggerFlag);
}

bool MemoryCache::hasTriggerAshmemMonitor()
{
  return m_ashmemMonTriggerFlag > 0;
}
