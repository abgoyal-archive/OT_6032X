...
#if ENABLE(VISIBLE_IMAGE_MANAGEMENT)
static inline BitmapImage* toBitmapImage(Image* image)
{
  return static_cast<BitmapImage*>(image);
}
#endif
...
#if ENABLE(VISIBLE_IMAGE_MANAGEMENT)
        , m_visibleScreenRect(0, 0, 0, 0)
        , m_wasMonitored(false)
#endif
{
    setStatus(Unknown);
...
void CachedImage::data(PassRefPtr<SharedBuffer> data, bool allDataReceived)
{
    ...
    if (allDataReceived) {
#if ENABLE(VISIBLE_IMAGE_MANAGEMENT)
    	ImageSource::ImageType imageType = toBitmapImage(m_image.get())->imageSource().getImageType();
    	if (imageType > ImageSource::UNKNOWN)
    	   m_wasMonitored = true;
#endif
        setLoading(false);
        checkNotify();
...
bool CachedImage::isGifImage() {
	if (m_image && toBitmapImage(m_image.get())->imageSource().getImageType() == ImageSource::GIF)
        return true;
    return false;
}
...
#if ENABLE(VISIBLE_IMAGE_MANAGEMENT)
bool CachedImage::willRenderThisOnExpandedRect()
{
    IntRect originVisibleScreen = m_visibleScreenRect;
    m_visibleScreenRect.inflateX(originVisibleScreen.width() << 3);
    m_visibleScreenRect.inflateY(originVisibleScreen.height() << 3);
    ...
    m_visibleScreenRect = originVisibleScreen;
    return renderThis;
}

void CachedImage::visibleImageProcess(const IntRect& visibleScreenRect)
{
    if (!m_image || !wasMonitored())
        return;
    ImageSource::ImageType imageType = toBitmapImage(m_image.get())->imageSource().getImageType();
    m_visibleScreenRect = visibleScreenRect;
    switch (imageType) {
    case ImageSource::GIF:
#if ENABLE(IMPROVE_ANIMATED_GIF_PERFORMANCE)
        if (CachedImage::getEnableGifAnimation())
            m_image->startAnimation();
#endif
        break;
    case ImageSource::ASHMEM: {
        if (!memoryCache()->hasTriggerAshmemMonitor())
            return;
        if (!willRenderThisOnExpandedRect())
            toBitmapImage(m_image.get())->imageSource().clearPixelDataIfPossible();
        break;
    }
    }
}
